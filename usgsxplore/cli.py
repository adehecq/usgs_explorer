...
@click.option(
    ...
    "--filter",
    type=click.STRING,
    help="String representation of metadata filter")
...
def search(
    ...
    scene_filter = SceneFilter.from_args(location=location, bbox=bbox, max_cloud_cover=clouds, date_interval=interval_date,
                          meta_filter=filter)
    if output is None:
